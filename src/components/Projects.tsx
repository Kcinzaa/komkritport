...
import { ArrowUpRight, ExternalLink, Globe, BarChart3, UtensilsCrossed, Trees } from "lucide-react";
...
function PhoneFrame({ dark = true, children }: { dark?: boolean; children: React.ReactNode }) {
  return (
    <div
      ...
      style={{ border: "2.5px solid rgba(255,255,255,0.13)", background: dark ? "#0a0a0a" : "#f5f5f5" }}
    >
      <div className="absolute top-0 left-0 right-0 flex justify-center z-20 pt-2">
        <div className="w-14 h-4 rounded-b-2xl" style={{ background: dark ? "#060606" : "#e5e5e5", border: dark ? "1px solid rgba(255,255,255,0.06)" : "1px solid rgba(0,0,0,0.08)" }} />
      </div>
      <div className="absolute inset-0 pt-5 overflow-hidden">{children}</div>
      <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-10 h-1 rounded-full" style={{ background: dark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.2)" }} />
    </div>
  );
}
  ...
  return (
    <div className="w-full rounded-xl overflow-hidden border border-white/[0.1] shadow-xl shadow-black/40">
      <div className="flex items-center justify-between px-3 py-2 bg-[#1c1c2e] border-b border-white/[0.07]">
        ...
        <div className="flex gap-1.5">
          {[1, 2, 3].map((n) => <div key={n} className="w-3.5 h-3.5 rounded bg-white/[0.06] border border-white/[0.07]" />)}
        </div>
      </div>
      {children}
    </div>
...
function ProjectVisual({ project, size = "featured" }: { project: Project; size?: "featured" | "small" }) {
  const h = size === "featured" ? "h-[200px]" : "h-[148px]";

  if (project.mockupType === "browser") {
    return (
      <BrowserFrame url={project.mockupUrl ?? ""} color={project.color}>
        <div className={`relative ${h} overflow-hidden`}>
          <NextImage src={project.image} alt={project.title} fill className="object-cover object-top" />
        </div>
      </BrowserFrame>
    );
  }
  ...
  // dashboard
  return (
    <DashboardFrame url={project.mockupUrl ?? "Power BI"} color={project.color}>
      <div className={`relative ${h} overflow-hidden`}>
        <NextImage src={project.image} alt={project.title} fill className="object-cover object-top" />
      </div>
    </DashboardFrame>
...
function FeaturedCard({ project, onClick }: { project: Project; onClick: () => void }) {
  ...
  return (
    ...
    >
      {mouse && <div className="absolute inset-0 pointer-events-none z-0" style={{ background: `radial-gradient(350px circle at ${mouse.x}px ${mouse.y}px, ${project.color}0d, transparent 70%)` }} />}
      ...
      <div className="relative z-10 grid lg:grid-cols-2 min-h-[300px]">
        ...
        <div className="p-8 md:p-10 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-5">
              <div className="w-11 h-11 rounded-2xl border flex items-center justify-center" style={{ backgroundColor: project.color + "18", borderColor: project.color + "30", color: project.color }}>
                {projIcon[project.id]}
              </div>
              <div>
            ...
            <h3 className="text-3xl md:text-4xl font-black text-white tracking-tight mb-3 leading-tight">{project.title}</h3>
          ...
          <div className="flex items-center gap-6 pt-6 mt-4 border-t border-white/[0.06]">
            {[{ label: "บทบาท", value: project.role }, { label: "ระยะเวลา", value: project.duration }].map((s) => (
              <div key={s.label}>
        ...
        {/* Visual */}
        <div className="relative flex items-center justify-center p-8 border-t lg:border-t-0 lg:border-l border-white/[0.06]" style={{ background: `radial-gradient(ellipse at center, ${project.gradientFrom}, transparent 70%)` }}>
...
function SmallCard({ project, index, onClick }: { project: Project; index: number; onClick: () => void }) {
  ...
  return (
    ...
    >
      {mouse && <div className="absolute inset-0 pointer-events-none z-0" style={{ background: `radial-gradient(220px circle at ${mouse.x}px ${mouse.y}px, ${project.color}0d, transparent 70%)` }} />}

      {/* Visual */}
      <div className="relative z-10 flex items-center justify-center px-5 pt-5 pb-4 border-b border-white/[0.06] min-h-[190px]" style={{ background: `radial-gradient(ellipse at center, ${project.gradientFrom}, transparent 80%)` }}>
      ...
      <div className="relative z-10 p-6 flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {project.tags.length > 3 && <span className="px-2 py-0.5 rounded-md text-[10px] text-neutral-700 bg-white/[0.03] border border-white/[0.05]">+{project.tags.length - 3}</span>}
        </div>
        <div className="mt-4 pt-4 border-t border-white/[0.06] flex items-center justify-between">
          ...
          <span className="text-xs font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center gap-1" style={{ color: project.color }}>
            ดูเพิ่มเติม <ArrowUpRight size={12} />
          </span>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-wrap items-end justify-between gap-6 mb-14">
            <motion.div variants={fadeUp} custom={0} initial="hidden" animate={inView ? "visible" : "hidden"}>
              <SectionLabel>ผลงาน</SectionLabel>
              <h2 className="text-5xl md:text-[5.5rem] font-black tracking-[-0.04em] text-white leading-[0.88]">Projects</h2>
            </motion.div>
            <motion.div variants={fadeUp} custom={0.5} initial="hidden" animate={inView ? "visible" : "hidden"} className="flex flex-col items-end gap-2">
              <p className="text-sm text-neutral-500 max-w-[240px] text-right leading-relaxed">กดที่การ์ดเพื่อดูรายละเอียดโปรเจกต์เพิ่มเติม</p>
